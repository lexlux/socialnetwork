...
$("#imageUploadButton").click(() => {
    ...
    if(canvas == null) {
        alert("Не удалось загрузить аватарку. Убедитесь, что этот файл изображение.");
        return;
    }
...
$("#coverPhotoButton").click(() => {
    ...
    if(canvas == null) {
        alert("Не удалось загрузить обложку. Убедитесь, что этот файл изображение.");
        return;
    }
...
function createPostHtml(postData, largeFont = false) {
    ...
    if(isRetweet) {
        retweetText = `<span>
                        ...
                        Поделился от <a href='/profile/${retweetedBy}'>@${retweetedBy}</a>    
                    </span>`
    }
    ...
    if(postData.replyTo && postData.replyTo._id) {
        ...
        replyFlag = `<div class='replyFlag'>
                        Репост от <a href='/profile/${replyToUsername}'>@${replyToUsername}<a>
                    </div>`;
    ...
    if (postData.postedBy._id == userLoggedIn._id) {
        ...
        if (postData.pinned === true) {
            ...
            pinnedPostText = "<i class='fas fa-thumbtack'></i> <span>Закрепленный пост</span>";
        }
...
function timeDifference(current, previous) {
    ...
    if (elapsed < msPerMinute) {
        if(elapsed/1000 < 30) return "Только сейчас";
...
function outputPosts(results, container) {
    ...
    if (results.length == 0) {
        container.append("<span class='noResults'>Не опубликовано ни одного поста.</span>")
    }
}
...
function outputUsers(results, container) {
    ...
    if(results.length == 0) {
        container.append("<span class='noResults'>Никаких результатов</span>")
    }
}
...
function outputNotificationList(notifications, container) {
    ...
    if(notifications.length == 0) {
        container.append("<span class='noResults'>Нет уведомлений.</span>");
    }
}
...
function getNotificationText(notification) {
    ...
    if(!userFrom.firstName || !userFrom.lastName) {
        return alert("Данные пользователя не заполнены.");
    }
    ...
    if(notification.notificationType == "retweet") {
        text = `${userFromName} поделился вашим постом`;
    }
    else if(notification.notificationType == "postLike") {
        text = `${userFromName} понравился ваш пост`;
    }
    else if(notification.notificationType == "reply") {
        text = `${userFromName} прокомментировал ваш пост`;
    }
    else if(notification.notificationType == "follow") {
        text = `${userFromName} подписан на вас`;
    }
